from flask import Flask, render_template, request

app = Flask(__name__)

# Dicionário com as especialidades, médicos, CRM e planos aceitos
dados_especialidades = {
    "Cardiologia": [
        {"nome": "Dr. André Souza", "crm": "CRM/MG 18432", "planos": ["Unimed", "Amil", "SulAmérica"]},
        {"nome": "Dra. Fernanda Melo", "crm": "CRM/MG 22105", "planos": ["Bradesco Saúde", "Unimed"]},
    ],
    "Pediatria": [
        {"nome": "Dra. Carla Nunes", "crm": "CRM/MG 15780", "planos": ["Unimed", "Hapvida", "Amil"]},
        {"nome": "Dr. Lucas Ribeiro", "crm": "CRM/MG 31209", "planos": ["SulAmérica", "NotreDame"]},
    ],
    "Dermatologia": [
        {"nome": "Dra. Juliana Costa", "crm": "CRM/MG 29801", "planos": ["Amil", "Bradesco Saúde"]},
    ],
}


@app.route('/', methods=['GET', 'POST'])
def index():
    especialidade = None
    medicos = None
    erro = None

    if request.method == 'POST':
        digitada = request.form.get('especialidade', '').strip()

        if not digitada:
            erro = "Digite uma especialidade para buscar."
        else:
            # busca sem diferenciar maiúsculas/minúsculas
            encontrada = None
            for chave in dados_especialidades:
                if chave.lower() == digitada.lower():
                    encontrada = chave
                    break

            if encontrada:
                especialidade = encontrada
                medicos = dados_especialidades[encontrada]
            else:
                erro = f'Não encontramos a especialidade "{digitada}".'

    return render_template(
        'painel.html',
        especialidade=especialidade,
        medicos=medicos,
        erro=erro,
    )


if __name__ == '__main__':
    app.run(debug=True)
